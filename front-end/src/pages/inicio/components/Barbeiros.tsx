  ...
  return (
    <article className="flex items-center gap-4 rounded-xl border border-border bg-background p-4 text-left">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-muted">{barbeiro.descricao || 'Profissional da nossa equipe.'}</p>
      </div>
    </article>
  )
}
  ...
  return (
    <section className="px-5 py-14 sm:px-8 lg:px-12" id="barbeiro">
      <div className="mx-auto max-w-5xl text-center">
        ...
        <h2 className="mt-2 font-serif text-3xl font-bold sm:text-4xl">Escolha o melhor profissional</h2>
        {carregando ? (
          <p className="mt-9 text-sm text-muted" role="status">Carregando barbeiros...</p>
        ) : null}
        {comErro ? (
          <p className="mt-9 text-sm text-danger" role="alert">Não foi possível carregar os barbeiros.</p>
        ) : null}
        {!carregando && !comErro && itens.length === 0 ? (
          <p className="mt-9 text-sm text-muted">Nenhum barbeiro disponível no momento.</p>
        ) : null}
        {!carregando && !comErro && itens.length > 0 ? (
          <div className="mt-9 grid gap-4 md:grid-cols-3">
            {itens.map((barbeiro) => <CartaoBarbeiro barbeiro={barbeiro} key={barbeiro.id} />)}
          </div>
        ) : null}
      </div>
    </section>
