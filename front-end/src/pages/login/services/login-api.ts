...
import { CODIGOS_PAPEL, type CredenciaisLogin, type RespostaLogin, type TipoErroLogin } from '../types'

const MENSAGEM_CREDENCIAIS = 'Não foi possível entrar. Verifique seus dados e tente novamente.'
const MENSAGEM_INDISPONIVEL = 'O acesso está indisponível no momento. Tente novamente mais tarde.'
...
export async function autenticar(credenciais: CredenciaisLogin): Promise<RespostaLogin> {
  let resposta: Awaited<ReturnType<typeof autenticacaoControllerLogin>>

  try {
    resposta = await autenticacaoControllerLogin({ email: credenciais.email.trim(), senha: credenciais.senha })
  } catch {
  ...
  if (status === 429) {
    const tentarNovamenteEm = obterTempoDeEspera(new Response(null, { headers: resposta.headers }))
    const complemento = tentarNovamenteEm ? ` Aguarde ${tentarNovamenteEm} segundos.` : ''
    throw new ErroLogin('limite', `Muitas tentativas de acesso.${complemento}`.trim(), tentarNovamenteEm)
  }
  ...
  return { ...dados, usuario: { ...dados.usuario, permissoes: dados.usuario.permissoes ?? [] } }
}
