...
const beneficios = [
  ...
  "Horários e profissionais em um só lugar",
  "Acesso para clientes e equipe",
];
...
export function IdentidadeLogin() {
  return (
    <section className="relative overflow-hidden bg-primary px-7 py-9 text-white sm:px-10 lg:flex lg:min-h-[650px] lg:flex-col lg:justify-between lg:px-12 lg:py-12">
      <div
        aria-hidden="true"
        className="absolute -right-24 -top-24 size-72 rounded-full border border-secondary opacity-30"
      />
      <div
        aria-hidden="true"
        className="absolute -bottom-32 -left-24 size-80 rounded-full border border-secondary opacity-30"
      />
      ...
      >
        <span
          aria-hidden="true"
          className="grid size-12 place-items-center rounded-full border border-secondary text-2xl text-secondary"
        >
          ✂
        </span>
        <span className="leading-none">
          <strong className="block font-serif text-xl tracking-wide">
            CORTE CERTO
          </strong>
        ...
        <ul className="mt-8 hidden space-y-4 lg:block">
          {beneficios.map((beneficio) => (
            <li className="flex items-center gap-3 text-sm" key={beneficio}>
              <span
                aria-hidden="true"
                className="grid size-6 place-items-center rounded-full bg-secondary font-bold text-foreground"
              >
                ✓
              </span>
              {beneficio}
      ...
      <p className="relative mt-10 hidden text-xs text-secondary lg:block">
        Atendimento com cuidado em cada detalhe.
      </p>
    </section>
  );
}
